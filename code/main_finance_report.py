...
# print the header, exactly:   === FINANCE: Daily Sales Detail ===
...
#    call get_raw_sales_data(seed) and store the result in `raw_data`.

raw_data = get_raw_sales_data(seed)
# 2. Transform — clean it, then total it.
#    call clean_sales_data(raw_data) and store it in `clean_data`.
#     call calculate_total_revenue(clean_data) and store it in `total_revenue`.
clean_data = clean_sales_data(raw_data)
total_revenue = calculate_total_revenue(clean_data)
# 3. Load — put it in front of a human.
#     call print_sales_table(clean_data).
#     print() a blank line.
